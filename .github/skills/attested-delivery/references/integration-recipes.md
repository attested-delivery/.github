...
`sarif-artifact` / `sarif-filename` outputs; the caller turns each verdict into a
signed, digest-bound attestation via `reusable-attest-scan.yml`. Example for SAST
(repeat per in-scope gate with that gate's predicate type):
...
jobs:
  sast:
    permissions:
      ...
      actions: read
      packages: read
    uses: <org>/.github/.github/workflows/reusable-sast-codeql.yml@<full-sha> # vX.Y.Z
    with:
      languages: javascript-typescript

  sast-attest:
    needs: [build, sast]     # build = the caller's image-build job that outputs image-digest
    permissions:
      id-token: write
      attestations: write
      contents: read
    uses: <org>/.github/.github/workflows/reusable-attest-scan.yml@<full-sha> # vX.Y.Z
    with:
...
entry point — the build stays in the caller; the central reusables sign and verify.
